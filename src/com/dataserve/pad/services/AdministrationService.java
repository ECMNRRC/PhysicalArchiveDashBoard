package com.dataserve.pad.services;
...
import com.dataserve.pad.business.audit.AuditManager;
import com.dataserve.pad.db.command.Command;
 ...
 */
public class AdministrationService extends PluginService {
	 ...
	 */
		...
		public void execute(PluginServiceCallbacks callbacks, HttpServletRequest request, HttpServletResponse response)	throws Exception {
			...
			try {
				Class<?> commandClass = Class.forName("com.dataserve.pad.db.command." + method);
				...
				if (comm.isAuditable()) {
					final CommandBase cBase = (CommandBase) comm;
					final String params = getParams(request);
					new Thread(new Runnable() {
						@Override
						public void run() {
							try {
								AuditBean bean = cBase.getAuditBean();
								bean.setMethod(method);
								bean.setParams(params);
								AuditManager am = new AuditManager();
								am.logAudit(bean);
							} catch (Exception e) {
								log.logError(this, method, "Failed to audit command");
								e.printStackTrace();
							}
						}
					}).start();
		...
		private String getParams(HttpServletRequest request) {
			...
			String ignoreList = "desktop,security_token";
